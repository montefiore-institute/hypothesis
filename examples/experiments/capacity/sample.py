...
def main(arguments):
    ...
    if not os.path.exists(path) or arguments.force:
        ...
        save_result(n_hidden, result_lf, name=name)
    else:
    ...
    plt.minorticks_on()
    plt.legend()
    plt.savefig("plots/hidden" + n_hidden + "/" +str(arguments.observations) + ".pdf", bbox_inches="tight", pad_inches=0)
    plt.clf()

def save_result(n_hidden, result, name):
    path = "results/hidden" + n_hidden + "/"
