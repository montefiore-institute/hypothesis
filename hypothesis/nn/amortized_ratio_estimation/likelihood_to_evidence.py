...
class LikelihoodToEvidenceSwappingCriterion(BaseCriterion):
    ...
    def _forward_without_logits(self, inputs, outputs):
        # A pass
        y_dependent, _ = self.estimator(inputs, outputs) # Dependent samples
        outputs_random = outputs[torch.randperm(self.batch_size)].detach()
        y_independent, _ = self.estimator(inputs, outputs_random) # Independent samples
        ...
        y_independent, _ = self.estimator(inputs_random, outputs) # Independent samples
    ...
    def _forward_with_logits(self, inputs, outputs):
        # A pass
        _, y_dependent = self.estimator(inputs, outputs) # Dependent samples
        outputs_random = outputs[torch.randperm(self.batch_size)].detach()
        _, y_independent = self.estimator(inputs, outputs_random) # Independent samples
        loss_a = self.criterion(y_dependent, self.ones) + self.criterion(y_independent, self.zeros)
        # B pass
        _, y_dependent = self.estimator(inputs, outputs) # Dependent samples
        inputs_random = inputs[torch.randperm(self.batch_size)].detach()
        _, y_independent = self.estimator(inputs_random, outputs) # Independent samples
        loss_b = self.criterion(y_dependent, self.ones) + self.criterion(y_independent, self.zeros)
        # Combined
        loss = (loss_a + loss_b) / 2
