...
class MLP(torch.nn.Module):
    ...
    def __init__(self, shape_xs,
        ...
        :param activation: An allocator which, when called, returns a :mod:`torch` activation.
        ...
        for index in range(1, len(layers)):
            layer = self._make_layer(activation, dropout, layers[index - 1], layers[index])
            mappings.append(self._make_layer)
        mappings.append(activation())
        mappings.append(torch.nn.Linear(layers[-1], self._dimensionality_ys))
        operation = allocate_output_transform(transform_output, self._dimensionality_ys)
