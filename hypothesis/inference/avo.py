...
class AdversarialVariationalOptimizationPlus(Method):
    ...
    A modified implementation of arxiv.org/abs/1707.07113
    """
    ...
    def allocate_optimizers(self):
        ...
        self.optimizer_proposal = torch.optim.RMSprop(
            [self.proposal.parameters()[0]], lr=self.lr_proposal)
        # Call the reset hook.
    ...
    def update_proposal(self, observations, thetas, x_thetas):
        ...
        p = (self.proposal.log_prob(thetas) - 1).exp().view(-1, 1)
        probabilities = torch.cat([p, p, p], dim=1)
        locs = loc.repeat(1, n).view(n, -1)
        gradients = (thetas - loc)
        d = (self.discriminator(sample(observations, self.batch_size)).mean() - self.discriminator(x_thetas))
        grad = (gradients * d).mean(dim=0).view(-1).detach()
        self.optimizer_proposal.zero_grad()
        loc.grad = grad.detach()
        self.optimizer_proposal.step()
        print(self.proposal.mu)
    ...
    def infer(self, observations, **kwargs):
        ...
        hypothesis.call_hooks(hypothesis.hooks.post_inference, self, proposal=final_proposal)
...
class NashBaseline(Baseline):
    ...
    def apply(self, inputs, gradients):
        ...
        with torch.no_grad():
            y = (1 - self.discriminator(inputs) + epsilon).log()
