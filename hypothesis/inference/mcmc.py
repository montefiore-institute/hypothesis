...
class LikelihoodFreeMetropolisHastings(MarkovChainMonteCarlo):
    ...
    def step(self, observations, theta):
        ...
        with torch.no_grad():
            ...
            in_theta = torch.tensor([theta] * num_observations)
            in_theta = in_theta.view(num_observations, -1)
            in_theta_next = torch.tensor([theta_next] * num_observations)
            in_theta_next = in_theta_next.view(num_observations, -1)
            in_current = torch.cat([in_theta, observations], dim=1)
            in_next = torch.cat([in_theta_next, observations], dim=1)
            s = self.classifier(in_current).log().sum() - self.classifier(in_next).log().sum()
            lr = (1 - s) / (s + epsilon)
            if not self.transition.is_symmetric():
                t_theta_next = self.transition.log_prob(theta, theta_next).exp()
                t_theta = self.transition.log_prob(theta_next, theta).exp()
